...
export function TodayPage() {
  return (
    <div>
      ...
      <section aria-label="Hotel overview" className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {todayMetrics.map((metric) => <MetricCard key={metric.label} metric={metric} />)}
      </section>

      <div className="mt-3 grid gap-3 xl:grid-cols-12">
        <div className="xl:col-span-4"><ArrivalsList arrivals={arrivals} /></div>
        <div className="xl:col-span-3"><NeedsAction items={needsAction} /></div>
        <div className="xl:col-span-5"><StayTimeline items={stayTimeline} /></div>
      </div>
    </div>
  )
}
