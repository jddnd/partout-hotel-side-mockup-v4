...
export function StayTimeline({ items }: Readonly<{ items: StayTimelineItem[] }>) {
  return (
    <section className="h-full rounded-card border border-partout-border bg-partout-surface p-4 shadow-card">
      ...
      <div className="mt-3 grid grid-cols-[104px_minmax(0,1fr)] gap-x-3">
        <div />
        ...
        {items.map((item) => (
          <div key={item.name} className="contents">
            ...
            <div className="relative h-10 border-b border-partout-border/70 bg-[linear-gradient(to_right,transparent_calc(14.285%-1px),#eceeea_14.285%,transparent_calc(14.285%+1px))] bg-[length:14.285%_100%]">
              <div className="absolute inset-x-1 top-[18px] h-1.5 rounded-full bg-partout-timeline" aria-hidden="true" />
              <div className="absolute left-[28.57%] top-0 h-full w-px bg-partout-today/75" aria-hidden="true" />
              <div
                className={item.state === 'confirmed' ? 'absolute top-[18px] h-1.5 rounded-full bg-partout-action' : 'absolute top-[18px] h-1.5 rounded-full bg-partout-checked'}
