  ...
  return (
    <section className="h-full rounded-card border border-partout-border bg-partout-surface p-4 shadow-card">
      <div className="flex items-center justify-between gap-4">
        ...
        <a href="/hotel/stays" className="text-[9px] font-medium text-partout-text-muted transition-colors hover:text-partout-action">View all arrivals</a>
      </div>
      <ul className="mt-3 divide-y divide-partout-border/80">
        {arrivals.map((arrival) => (
          <li key={arrival.id}>
            <a
              href={`/hotel/stays/${arrival.id}`}
              className="grid min-h-14 grid-cols-[40px_minmax(0,1fr)] items-center gap-x-2 py-1.5 transition-colors hover:text-partout-action xl:grid-cols-[40px_minmax(86px,1fr)_auto_auto_auto]"
            >
