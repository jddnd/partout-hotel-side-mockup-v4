...
export function HotelSidebar() {
  return (
    <aside className="hidden min-h-screen bg-partout-forest text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:px-4 lg:py-5">
      <div className="px-1 text-[17px] font-medium leading-none tracking-[-0.025em]">partout</div>

      <button
        type="button"
        className="mt-5 flex h-9 w-full items-center justify-between rounded-control border border-white/10 bg-white/[0.035] px-3 text-left text-[10px] font-medium text-white/86 transition-colors hover:bg-white/[0.07]"
      >
      ...
      <nav aria-label="Hotel workspace" className="mt-5 space-y-1">
        {navItems.map(({ label, to, icon: Icon }) => (
          <Link
            ...
            className="flex h-9 items-center gap-3 rounded-control px-3 text-[11px] font-medium text-white/74 transition-colors hover:bg-white/[0.07] hover:text-white"
