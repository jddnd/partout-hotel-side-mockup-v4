    ...
    <section className="overflow-hidden rounded-card border border-partout-border bg-partout-surface shadow-card" aria-labelledby="active-stays-title">
      <div className="flex items-center justify-between border-b border-partout-border px-4 py-3">
        <div>
          ...
          <p className="mt-1 text-[7px] text-partout-text-muted">People you are hosting, with the collaboration context kept close</p>
        </div>
        ...
        {stays.map((stay) => {
          ...
          return (
            <a key={stay.id} href={`/hotel/stays/${stay.id}`} className="block text-inherit no-underline">
              <article className={`grid min-h-[82px] grid-cols-[minmax(180px,1.35fr)_minmax(120px,1fr)_92px_118px_24px] items-center gap-3 px-4 py-3 transition-colors ${selected ? 'bg-partout-canvas' : 'hover:bg-partout-canvas/55'}`}>
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate text-[7px] text-partout-text-muted">{stay.relationshipLabel}</p>
                </div>
